    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.get_by_test_id("project-title-input").fill("雾中来信")
                page.get_by_test_id("project-slug-input").fill("fog-letter")
                page.get_by_test_id("project-genre-input").fill("都市悬疑")
                page.get_by_test_id("project-chapter-title-input").fill("第一章 门缝")
                page.get_by_test_id("project-premise-input").fill("一个人收到无法追踪的信后重新检查记忆。")
                page.get_by_test_id("project-style-input").fill("克制，少解释心理，多用动作和物件承压。")
                opening = "我站在门口，心里很乱，想起过去很多事情。"
                page.get_by_test_id("project-opening-input").fill(opening)
