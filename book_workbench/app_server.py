...
<body data-app-title="BookWorkbench Local App">
  ...
  <script>
    ...
    const state = { workspace: null, projects: [], project: null, annotations: [], audit: [], currentFile: null, currentChapter: null, lastPatch: null, lastPreview: null, activeView: "dashboard", selectedAnnotationId: null };
    ...
    function setView(view) { if (view !== "dashboard" && view !== "settings" && !hasProject()) { toast("请先新建或打开一个项目。"); view = "dashboard"; } state.activeView = view; document.querySelectorAll(".view").forEach((el) => el.classList.add("hidden")); const target = $(`view-${view}`); if (target) target.classList.remove("hidden"); document.querySelectorAll(".nav button").forEach((btn) => btn.classList.toggle("active", btn.dataset.view === view)); const titles = { dashboard: hasProject() ? [bookTitle(), "项目总览 · 点击章节进入文稿"] : ["项目", "还没有打开书稿。请从项目列表进入，或新建一个项目。"], editor: ["文稿编辑", `项目 / ${bookTitle()} / 文稿`], annotations: ["批注中心", "集中处理开放批注与 AI 建议"], diff: ["Patch / Diff 审核", "所有修改先预览，再由 Runtime 安全应用"], rules: ["规则中心", "规则只传播到 draft / unreviewed，locked 永不修改"], export: ["导出项目", "Markdown 项目结构可直接复制或进入导出适配器"], settings: ["设置 / 安全中心", "安全边界、Codex 连接与审计策略"] }; const [title, subtitle] = titles[view] || titles.dashboard; $("pageTitle").textContent = title; $("pageSubtitle").textContent = subtitle; if (view === "dashboard") renderDashboard(); if (view === "rules") renderRules(); if (view === "annotations") renderAnnotations(); if (view === "diff") renderDiffIfReady(); }
    async function loadHealth() { const health = await api("/api/health"); const codexOk = !!health.codex?.ok; const runtimeOk = !!health.runtime?.ok; $("settingsCodex").textContent = codexOk ? "OK" : (health.codex?.status || "未连接"); $("settingsCodex").className = codexOk ? "ok" : "bad"; if (hasProject()) $("pageSubtitle").innerHTML = `Runtime <span class="${runtimeOk ? "ok" : "bad"}">${runtimeOk ? "OK" : "未打开项目"}</span> · Codex <span class="${codexOk ? "ok" : "bad"}">${codexOk ? "OK" : "未连接"}</span>`; return health; }
    ...
    async function openProject(relativePath) { const payload = await api("/api/projects/open", { method: "POST", body: JSON.stringify({ relativePath }) }); state.project = payload.project; state.currentFile = null; state.currentChapter = null; state.lastPatch = null; state.lastPreview = null; await loadProjectDetails(true); setView("dashboard"); toast(`已打开：${bookTitle()}`); }
    async function loadProjectDetails(loadFirst = true) { if (!hasProject()) return null; const [project, annotationPayload, auditPayload] = await Promise.all([api("/api/project"), api("/api/annotations?include_resolved=1"), api("/api/audit")]); state.project = project; state.annotations = annotationPayload.annotations || []; state.audit = auditPayload.events || []; state.selectedAnnotationId = state.annotations[0]?.id || null; state.currentFile = state.currentFile || firstChapterFile(); renderShell(); renderDashboard(); renderRules(); renderAnnotations(); if (loadFirst && state.currentFile) await loadChapter(state.currentFile); return project; }
...
class RuntimeWebApp:
    ...
    def health(self) -> Dict[str, Any]:
        ...
        codex = self.codex_client.health() if self.project_root is not None else {
            "ok": False,
            "status": "not_checked_until_project_open",
