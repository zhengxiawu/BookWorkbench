...
<body data-app-title="书稿工作台本地版">
  ...
  <div class="modal-backdrop hidden" id="projectModal" data-testid="new-project-modal">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="projectModalTitle">
      ...
      <form id="projectForm" autocomplete="off">
        <label class="field" for="projectModeInput">写作模式<select id="projectModeInput" name="mode" aria-label="写作模式" data-testid="project-mode-input"><option value="standard">普通书稿项目</option><option value="powerbook-guide">PowerBook / Codex 写书闭环</option></select><span class="field-help">PowerBook 模式会创建项目约定、写作流程、理论定义、证据登记和审阅资料，并生成可继续修订的完整第一章草稿。</span></label>
        <div class="form-grid">
        ...
        <label class="field" for="projectOpeningInput">开篇正文 / 初始输入（可选；普通模式留空不会预置小说）<textarea id="projectOpeningInput" name="openingText" autocomplete="off" aria-label="开篇正文" data-testid="project-opening-input" placeholder="普通项目可以留空；PowerBook 模式可粘贴你最开始给 Codex 的完整写书输入。"></textarea></label>
      </form>
  ...
  <script>
    ...
    function workflowTargetFile() { const workflow = state.project?.powerbookWorkflow || {}; const preferred = workflow.chapterTarget || workflow.firstChapter || ""; const files = Object.keys(state.project?.blocks || {}); if (state.currentFile && files.includes(state.currentFile)) return state.currentFile; if (preferred && files.includes(preferred)) return preferred; return firstChapterFile(); }
    ...
    async function openProject(relativePath, options = {}) { const payload = await api("/api/projects/open", { method: "POST", body: JSON.stringify({ relativePath }) }); state.project = payload.project; state.annotations = payload.project.annotations || []; state.discussions = []; state.audit = []; renderShell(); state.currentFile = null; state.currentChapter = null; state.lastPatch = null; state.lastPreview = null; state.lastWorkflow = null; setView("dashboard"); const sidecars = loadSidecars().catch(showError); if (options.awaitSidecars) await sidecars; loadHealth().catch(() => {}); if (!options.quiet) toast(`已打开：${bookTitle()}`); return state.project; }
    ...
    async function runPowerBookGeminiChapter(options = {}) { if (!state.project?.powerbookWorkflow) throw new Error("当前项目不是专用写作项目。"); const file = workflowTargetFile(); if (!file) throw new Error("当前项目没有可修订章节。"); const targetLabel = fileDisplayLabel(file); state.workflowRunning = true; state.workflowStatus = `正在处理 ${targetLabel}`; renderDashboard(); const chapterBefore = await api("/api/chapters/" + encodeURIComponent(file)); state.currentFile = file; state.currentChapter = chapterBefore; toast(options.directGemini ? `正在通过受信任入口尝试调用 Gemini：${targetLabel}` : `正在按工作流生成整章修改建议：${targetLabel}`); try { const result = await api("/api/workflows/powerbook/gemini-chapter", { method: "POST", body: JSON.stringify({ file, timeoutSeconds: 90, directGemini: !!options.directGemini, instruction: "按 PowerBook 原始 Codex/Gemini 流程修订当前章节；只输出 PatchProposal。" }) }); state.lastPatch = result.output; state.lastWorkflow = result.workflow || result.output?.workflow || null; state.currentChapter = chapterBefore; const workflow = state.lastWorkflow || {}; const invoked = workflow.geminiInvoked ? "已实际调用 Gemini" : (workflow.localFallback ? "外部模型不可用，已生成本地安全兜底" : "未实际调用 Gemini，已保留原因"); toast(`${invoked}，进入差异审核。`); await previewPatch(false); setView("diff"); return result; } finally { state.workflowRunning = false; state.workflowStatus = null; renderDashboard(); } }
...
class RuntimeWebApp:
    ...
    def powerbook_gemini_chapter(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        timeout = _optional_number(payload, "timeoutSeconds") or 90.0
        ...
        try:
            ...
            if validation_is_valid(local_validation) and patch_has_changes(local_patch):
                ...
                return {
                    ...
                    "events": [{"type": "powerbook.local_fallback.patch.ready", "summary": codex_summary, "workflow": local_patch["workflow"]}],
                    "output": local_patch,
                    "codex": codex_summary,
                    "fallbackFrom": {"source": workflow.get("source"), "reason": workflow.get("fallbackReason")},
                }
            workflow["localFallbackError"] = "local_patch_failed_runtime_validation"
        except Exception as exc:
