...
from .project import ProjectLoadError, index_markdown_blocks, load_project, safe_chapter_path, write_block_index
from .project_creator import ProjectCreationError, create_book_project
from .yaml_lite import load_chapter_status
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(174px, 1fr)); gap: 16px; }
    ...
    .metric { padding: 18px; min-height: 148px; display: flex; flex-direction: column; align-items: flex-start; justify-content: space-between; gap: 12px; }
    ...
    .metric .label { color: var(--muted); font-size: 14px; line-height: 1.25; white-space: nowrap; }
    .metric .num { display: block; margin: 4px 0; font-size: 36px; line-height: 1; font-weight: 800; letter-spacing: -.04em; }
    .metric .note { display: block; font-size: 13px; line-height: 1.35; overflow-wrap: anywhere; }
    .metric-body { min-width: 0; width: 100%; display: grid; gap: 2px; }
...
<body data-app-title="BookWorkbench Local App">
    ...
    <main class="main">
      ...
      <section id="view-editor" class="view grid editor hidden">
        ...
        <aside class="side-stack">
          <section class="card panel"><div class="panel-title"><h3>批注与 AI 建议</h3><button class="ghost-btn" id="addAnnotationGlobalBtn">添加批注</button></div><div class="tabs"><button class="tab active" data-annotation-tab="annotations">批注 (<span id="annotationCount">0</span>)</button><button class="tab" data-annotation-tab="suggestions">AI 建议</button></div><div id="annotationPanel" class="mt"></div></section>
      ...
      <section id="view-diff" class="view grid diff hidden">
        <div>
          ...
          <section class="card diff-reason" id="diffReasonCard"><div class="panel-title"><h3>ⓘ 修改原因 / Runtime 校验</h3><button class="link-btn" id="toggleDiffReasonBtn" aria-expanded="true">收起</button></div><div class="diff-body"><p id="changeReason" class="muted">所有 AI 输出只能在这里预览，接受后才写入。</p><pre class="diff-raw" id="rawDiff"></pre></div></section>
        </div>
      ...
      <section id="view-settings" class="view hidden"><section class="card panel"><h2>设置 / 安全中心</h2><div class="check-list"><div class="check-row"><span>🛡</span><span>AI 只生成建议，不自动写入</span><span class="status-badge">开启</span></div><div class="check-row"><span>🔒</span><span>锁定章节禁止修改</span><span class="status-badge">开启</span></div><div class="check-row"><span>▣</span><span>修改必须先生成 PatchProposal</span><span class="status-badge">开启</span></div><div class="check-row"><span>⌘</span><span>Codex app-server</span><span class="status-badge neutral" id="settingsCodex">未检测</span></div><div class="check-row"><span>□</span><span>Workspace</span><strong class="path-inline" id="settingsWorkspace" title="">—</strong></div></div></section></section>
    </main>
  </div>
  ...
  <script>
    ...
    function ruleTypeLabel(type) { return ({ all: "全部", style: "风格", structure: "结构", setting: "设定", continuity: "设定" })[type] || type || "未分类"; }
    ...
    async function loadHealth() { const health = await api("/api/health"); const codexOk = !!health.codex?.ok; const runtimeOk = !!health.runtime?.ok; const codexLabel = codexStatusLabel(health.codex); $("settingsCodex").textContent = codexLabel; $("settingsCodex").title = codexLabel; $("settingsCodex").className = `status-badge ${codexOk ? "" : "neutral"}`; if (hasProject()) $("pageSubtitle").innerHTML = `Runtime <span class="${runtimeOk ? "ok" : "bad"}">${runtimeOk ? "已连接" : "未打开项目"}</span> · Codex <span class="${codexOk ? "ok" : "bad"}">${escapeHtml(codexLabel)}</span>`; return health; }
    async function loadWorkspace() { const [workspace, projects, project] = await Promise.all([api("/api/workspace"), api("/api/projects"), maybeApi("/api/project")]); state.workspace = workspace; state.projects = projects.projects || []; if (project && project.open !== false) state.project = project; $("settingsWorkspace").textContent = workspace.root || "—"; $("settingsWorkspace").title = workspace.root || ""; $("workspaceShort").textContent = workspace.root ? workspace.root.split("/").slice(-1)[0] : "本地"; $("workspaceBar").style.width = `${Math.min(100, state.projects.length * 12)}%`; renderShell(); if (hasProject()) await loadProjectDetails(false); else renderDashboard(); }
    ...
    async function openProject(relativePath, options = {}) { const payload = await api("/api/projects/open", { method: "POST", body: JSON.stringify({ relativePath }) }); state.project = payload.project; state.annotations = payload.project.annotations || []; state.discussions = []; state.audit = []; renderShell(); state.currentFile = null; state.currentChapter = null; state.lastPatch = null; state.lastPreview = null; setView("dashboard"); if (options.awaitSidecars) await loadSidecars(); loadHealth().catch(() => {}); if (!options.quiet) toast(`已打开：${bookTitle()}`); return state.project; }
    ...
    function renderAnnotationPanel() { const list = state.annotations.filter((item) => !state.currentFile || item.file === state.currentFile); const openCount = list.filter((item) => item.status === "open").length; $("annotationCount").textContent = list.length; $("pendingAnnotationCount").textContent = openCount; document.querySelectorAll("[data-annotation-tab]").forEach((tab) => tab.classList.toggle("active", tab.dataset.annotationTab === state.activeAnnotationTab)); if (state.activeAnnotationTab === "suggestions") { $("annotationPanel").innerHTML = `<div class="ai-item"><span class="small-icon purple">✦</span><div><strong>${openCount ? `${openCount} 条批注可处理` : "暂无待处理批注"}</strong><br><span class="muted">点击“处理当前批注”后只生成 PatchProposal。</span></div><span class="tag">建议</span></div><div class="ai-item"><span class="small-icon blue">▣</span><div><strong>安全边界</strong><br><span class="muted">用户接受前正文不写入；hash 漂移会被拒绝。</span></div><span class="tag">Runtime</span></div>`; } else { $("annotationPanel").innerHTML = list.length ? list.map((a) => `<button class="annotation-card ${a.id === state.selectedAnnotationId ? "active" : ""}" data-annotation="${escapeHtml(a.id)}"><div class="annotation-head"><strong>${escapeHtml(a.id)}</strong><span>${escapeHtml(a.file)} · ${escapeHtml(a.block_id)}</span></div><p>${escapeHtml(a.text)}</p><div class="annotation-head"><span>${escapeHtml(a.status)}</span><span>选择处理 ›</span></div></button>`).join("") : `<div class="empty-state" style="min-height:180px"><div><div class="empty-icon">☵</div><h2>当前章节暂无批注</h2><p>选中文稿中的句子，点击“添加批注”。批注会写入 .bookai/annotations.jsonl，不污染正文。</p></div></div>`; } document.querySelectorAll("[data-annotation]").forEach((el) => el.addEventListener("click", () => { state.selectedAnnotationId = el.dataset.annotation; state.activeAnnotationTab = "annotations"; renderAnnotationPanel(); })); $("aiActionList").innerHTML = `<div class="ai-item"><span class="small-icon purple">✦</span><div><strong>局部改写</strong><br><span class="muted">只生成 PatchProposal，用户接受前不写正文</span></div><span class="tag">安全</span></div><div class="ai-item"><span class="small-icon blue">▣</span><div><strong>锚点校验</strong><br><span class="muted">beforeHash 不匹配时拒绝自动应用</span></div><span class="tag">强制</span></div>`; }
    function renderAnnotationCards(list, max = 80) { const visible = list.slice(0, max); const omitted = Math.max(0, list.length - visible.length); const note = omitted ? `<div class="annotation-list-note">已显示前 ${visible.length} / ${list.length} 条批注，打开具体章节可继续缩小范围。</div>` : ""; return note + visible.map((a) => `<div class="annotation-card"><div class="annotation-head"><strong>${escapeHtml(a.id)}</strong><span>${escapeHtml(a.file)} · ${escapeHtml(a.block_id)}</span></div><p>${escapeHtml(a.text)}</p><span class="status-pill status-${a.status === "open" ? "draft" : "reviewed"}">${escapeHtml(a.status)}</span></div>`).join(""); }
    function renderAnnotations() { $("annotationCenter").innerHTML = state.annotations.length ? renderAnnotationCards(state.annotations, 80) : `<div class="empty-state" style="min-height:260px"><div><div class="empty-icon">☵</div><h2>暂无批注</h2><p>批注会保存在 sidecar 文件中，不会插入正文。</p></div></div>`; }
...
class RuntimeWebApp:
    ...
    def chapter(self, file_path: str) -> Dict[str, Any]:
        ...
        chapter_path = safe_chapter_path(self.project_root, rel_path)
        blocks = index_markdown_blocks(self.project_root, rel_path)
        if not blocks:
            raise ProjectLoadError(f"Unknown chapter: {rel_path}")
        status_path = self.project_root / ".bookai" / "chapter-status.yaml"
        chapter_status = load_chapter_status(status_path.read_text(encoding="utf-8") if status_path.exists() else "")
        ordered_blocks = dict(sorted((block_id, asdict(block)) for block_id, block in blocks.items()))
        chapter_title = _markdown_title(chapter_path.read_text(encoding="utf-8"), rel_path)
        return {
            "file": rel_path,
            "title": chapter_title,
            "status": chapter_status.get(rel_path, "draft"),
