...
from .autonomous_workflow import load_run_artifacts, load_run_patch, load_run_status, run_autonomous_workflow
...
DEFAULT_POWERBOOK_WORKFLOW_TIMEOUT_SECONDS = 180.0
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .field input, .field textarea, .field select { width: 100%; border: 1px solid #dfe5f0; border-radius: 12px; padding: 12px; outline: none; background: #fff; color: var(--text); }
    .field textarea { min-height: 92px; resize: vertical; line-height: 1.6; }
    .field-help { color: var(--muted); font-size: 13px; font-weight: 500; line-height: 1.55; }
...
<body data-app-title="书稿工作台本地版">
  ...
  <div class="modal-backdrop hidden" id="projectModal" data-testid="new-project-modal">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="projectModalTitle">
      ...
      <form id="projectForm" autocomplete="off">
        <label class="field" for="projectModeInput">写作模式<select id="projectModeInput" name="mode" aria-label="写作模式" data-testid="project-mode-input"><option value="standard">普通书稿项目</option><option value="powerbook-guide">自主写书闭环</option></select><span class="field-help">自主模式会创建项目约定、写作流程、理论定义、证据登记和审阅资料，并生成可继续修订的完整第一章草稿。</span></label>
        <div class="form-grid">
        ...
        <label class="field" for="projectOpeningInput">开篇正文 / 初始输入（可选；普通模式留空不会预置小说）<textarea id="projectOpeningInput" name="openingText" autocomplete="off" aria-label="开篇正文" data-testid="project-opening-input" placeholder="普通项目可以留空；自主模式可粘贴你最开始给智能服务的完整写书输入。"></textarea></label>
      </form>
  ...
  <script>
    ...
    function firstChapterFile() { const files = Object.keys(state.project?.blocks || {}).sort(); return files.find((file) => /ch0*1/i.test(file) && !/ch0*0/i.test(file)) || files.find((file) => !/ch0*0/i.test(file)) || files[0] || null; }
    function workflowTargetFile() { const workflow = state.project?.powerbookWorkflow || {}; const preferred = workflow.chapterTarget || workflow.firstChapter || ""; const files = Object.keys(state.project?.blocks || {}); if (state.workflowSelectedFile && files.includes(state.workflowSelectedFile)) return state.workflowSelectedFile; if (preferred && files.includes(preferred)) return preferred; return firstChapterFile(); }
    ...
    async function loadProjectDetails(loadFirst = true) { if (!hasProject()) return null; const [project, discussionPayload, auditPayload] = await Promise.all([api("/api/project"), api("/api/discussions"), api("/api/audit")]); const previousAnnotationId = state.selectedAnnotationId; state.project = project; state.annotations = project.annotations || []; state.discussions = discussionPayload.discussions || []; state.audit = auditPayload.events || []; const files = Object.keys(state.project?.blocks || {}); const openAnnotations = state.annotations.filter((item) => item.status === "open"); state.selectedAnnotationId = openAnnotations.find((item) => item.id === previousAnnotationId)?.id || openAnnotations[0]?.id || null; state.currentFile = state.currentFile || firstChapterFile(); if (state.project?.powerbookWorkflow && (!state.workflowSelectedFile || !files.includes(state.workflowSelectedFile))) state.workflowSelectedFile = workflowTargetFile(); renderShell(); renderDashboard(); renderRules(); if (state.activeView === "annotations") renderAnnotations(); if (state.activeView === "discussions") renderDiscussions(); if (loadFirst && state.currentFile) await loadChapter(state.currentFile); await loadHealth().catch(() => {}); return project; }
    ...
    async function runAutonomousWorkflow(options = {}) { if (!state.project?.powerbookWorkflow) throw new Error("当前项目不是专用写作项目。"); const file = workflowTargetFile(); if (!file) throw new Error("当前项目没有可修订章节。 "); const targetLabel = fileDisplayLabel(file); state.workflowSelectedFile = file; state.workflowRunning = true; state.workflowStatus = `准备隔离副本自主工作流：${targetLabel}`; renderDashboard(); const chapterBefore = await api("/api/chapters/" + encodeURIComponent(file)); state.currentFile = file; state.currentChapter = chapterBefore; state.workflowStatus = `智能服务正在隔离副本自主修订：${targetLabel}`; renderDashboard(); toast(`自主写作工作流已启动：${targetLabel}。正式书稿不会直接写入。`); try { const result = await api("/api/workflows/autonomous/start", { method: "POST", body: JSON.stringify({ file, timeoutSeconds: options.timeoutSeconds || 180, mode: options.mode || "powerbook-autonomous-chapter", goal: options.goal || "按原自主写书流程修订当前章节：先读项目约定、规则、作者批注和修订日志，在隔离副本里生成运行计划、规则变化、修订日志和质量报告，再输出可审核章节差异。" }) }); state.workflowStatus = "正在把隔离副本差异交给运行时校验"; renderDashboard(); state.lastPatch = result.output; state.lastWorkflow = result.workflow || result.output?.workflow || null; state.currentChapter = chapterBefore; const workflow = state.lastWorkflow || {}; if (workflow.diagnosticOnly || workflow.localFallback) { toast("自主工作流未产出可提交正文；已显示诊断和证据。"); } else { const quality = workflow.qualityReport || {}; toast(`自主工作流已生成 ${quality.changedBlocks || (result.output?.changes || []).length || 0} 个可审核修改，进入差异审核。`); } await previewPatch(false); setView("diff"); return result; } finally { state.workflowRunning = false; state.workflowStatus = null; renderDashboard(); } }
    ...
    function renderWorkflowEvidence() { const el = $("workflowEvidence"); if (!el) return; const workflow = state.lastWorkflow || state.lastPatch?.workflow; if (!workflow) { el.classList.add("hidden"); el.innerHTML = ""; return; } const invoked = workflow.geminiInvoked ? "是" : "否"; const sourceMap = { "gemini-3.1-pro-preview": "Gemini 直连", "codex-app-server": workflow.chunked ? "Codex 分段" : "Codex 本地服务", "local-workflow-fallback": "失败诊断", "autonomous-codex-scratch": "自主隔离副本工作流", "autonomous-workflow-diagnostic": "自主工作流诊断" }; const source = sourceMap[workflow.source] || workflow.source || "待确认"; const reason = workflow.fallbackReason ? `<div class="check-row"><span>!</span><span>失败 / 回退原因</span><strong>${escapeHtml(workflow.fallbackReason)}</strong></div>` : ""; const fallback = workflow.localFallback ? `<div class="check-row"><span>!</span><span>兜底状态</span><strong class="bad">仅诊断，不可提交</strong></div>` : ""; const target = workflow.file ? `<div class="check-row"><span>▤</span><span>目标章节</span><strong>${escapeHtml(fileDisplayLabel(workflow.file))}</strong></div>` : ""; const chunks = workflow.chunked ? `<div class="check-row"><span>≋</span><span>分段结果</span><strong>${escapeHtml(workflow.successfulChunkCount || 0)} / ${escapeHtml(workflow.chunkCount || 0)} 段</strong></div>` : ""; const workflowName = workflow.name || "PowerBook 受信任修订"; const log = workflow.runLog ? `<div class="check-row"><span>☰</span><span>运行日志</span><strong>${escapeHtml(workflow.runLog)}</strong></div>` : ""; const isolated = workflow.scratchPolicy ? `<div class="check-row"><span>✎</span><span>隔离副本边界</span><strong>${escapeHtml(workflow.scratchPolicy)}</strong></div>` : ""; const quality = workflow.qualityReport ? `<div class="check-row"><span>◆</span><span>质量报告</span><strong>${escapeHtml(workflow.qualityReport.changedBlocks || 0)} 块 / ${escapeHtml(workflow.qualityReport.changedChars || 0)} 字</strong></div>` : ""; const artifactRows = (workflow.artifacts || []).slice(0, 5).map((item) => `<div class="check-row"><span>📎</span><span>${escapeHtml(item.label || item.path)}<br><span class="muted">${escapeHtml(item.path || "")}</span></span><strong>已保存</strong></div>`).join(""); const scriptRow = workflow.scriptPath ? `<div class="check-row"><span>⌘</span><span>脚本</span><strong>${escapeHtml(workflow.scriptPath)}</strong></div>` : ""; el.classList.remove("hidden"); el.innerHTML = `<h3 class="mt">工作流证据</h3><div class="check-list"><div class="check-row"><span>▣</span><span>工作流</span><strong>${escapeHtml(workflowName)}</strong></div>${target}<div class="check-row"><span>✦</span><span>模型</span><strong>${escapeHtml(workflow.model || "codex-app-server")}</strong></div>${scriptRow}<div class="check-row"><span>▣</span><span>实际调用 Gemini</span><strong class="${workflow.geminiInvoked ? "ok" : "bad"}">${invoked}</strong></div><div class="check-row"><span>→</span><span>建议来源</span><strong>${escapeHtml(source)}</strong></div>${isolated}${quality}${chunks}${fallback}${reason}${log}${artifactRows}</div>`; }
...
原因：${workflow.fallbackReason || "外部模型未返回可用结果"}` : diffSafetySummary(file, displayBlock, valid, issues); $("diffReasonCard").classList.toggle("collapsed", !!state.diffReasonCollapsed); $("toggleDiffReasonBtn").textContent = state.diffReasonCollapsed ? "展开" : "收起"; $("toggleDiffReasonBtn").setAttribute("aria-expanded", String(!state.diffReasonCollapsed)); $("beforeLines").innerHTML = splitLines(before).map((line, idx) => `<div class="diff-line minus"><span class="ln">${idx + 1}</span><span>${escapeHtml(line)}</span><span>−</span></div>`).join(""); $("afterLines").innerHTML = splitLines(after).map((line, idx) => `<div class="diff-line plus"><span class="ln">${idx + 1}</span><span>${escapeHtml(line)}</span><span>＋</span></div>`).join(""); $("changeCheckList").innerHTML = diagnosticOnly ? `<div class="check-row"><span>!</span><span>没有可应用正文修改</span><strong class="bad">诊断</strong></div><div class="check-row"><span>↻</span><span>建议操作</span><strong>重试 / 缩小章节</strong></div>` : (issues.length ? issues.map((issue) => `<div class="check-row"><span>!</span><span>${escapeHtml(safetyIssueLabel(issue.code))}</span><strong class="bad">拒绝</strong></div>`).join("") : `<div class="check-row"><span>▤</span><span>修改的块<br><span class="muted">${escapeHtml(displayBlock)}</span></span>${statusBadge("通过")}</div><div class="check-row"><span>✦</span><span>将应用的规则</span><strong class="ok">${escapeHtml(ruleDisplayLabel((patch.rulesUsed || [])[0]))}</strong></div><div class="check-row"><span>🔒</span><span>锁定章节保持不变</span>${statusBadge("是")}</div>`); $("commitPreview").textContent = isRejected ? (diagnosticOnly ? "当前只是工作流失败诊断，不会写入或提交。" : "当前建议未通过运行时校验，不能写入或提交。") : `安全应用修改建议：${patchSourceLabel(source)}`; renderWorkflowEvidence(); }
    ...
    async function submitProject(event) { event.preventDefault(); const payload = Object.fromEntries(new FormData(event.target).entries()); const button = $("submitProjectBtn"); if (button) { button.disabled = true; button.textContent = "正在创建..."; } try { const result = await api("/api/projects/create", { method: "POST", body: JSON.stringify(payload) }); closeProjectModal(); await refreshProjects(); const relativePath = result.summary?.relativePath || result.plan?.slug || payload.slug; const baseline = result.baselineCommitCreated ? "，已建立版本基线" : ""; if (relativePath) { await openProject(relativePath, { quiet: true, awaitSidecars: true }); toast(`已创建并打开项目：${result.summary?.title || result.plan?.title || payload.title}${baseline}`); } else { toast(`已创建项目：${result.summary?.title || result.plan?.title || payload.title}${baseline}`); } } finally { if (button) { button.disabled = false; button.textContent = "创建项目"; } } }
...
class RuntimeWebApp:
    ...
    def powerbook_gemini_chapter(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        runtime = self._require_runtime()
        assert self.project_root is not None
        context = runtime.refreshed_context()
        if not powerbook_workflow_available(context.root):
            raise ProjectLoadError("当前项目不是专用写作项目，不能运行该写作流程。")
        file_path = _optional_string(payload, "file") or _optional_string(payload, "chapter")
        if not file_path:
            file_path = sorted(context.blocks)[0] if context.blocks else ""
        if not file_path:
        ...
        if prefer_direct_gemini:
            direct_attempted = True
            direct = run_powerbook_gemini_direct(
                context,
                file_path,
            ...
            direct_summary = {
                ...
                "stderr": direct.get("stderr", ""),
            }
            direct_workflow = direct.get("workflow")
            if isinstance(direct_workflow, dict):
                workflow.update(direct_workflow)
            proposal = direct.get("patchProposal")
            validation = runtime.validate_patch(proposal) if proposal is not None else {"valid": False, "issues": [{"code": "missing_patch", "message": "Gemini 没有生成可校验的修改建议。"}]}
            if direct.get("ok") and validation_is_valid(validation) and patch_has_changes(proposal):
                workflow["source"] = "gemini-3.1-pro-preview"
                workflow["runLog"] = workflow.get("runLog") or "Gemini 已返回有效整章修订；等待差异审核。"
                workflow["patchValidation"] = validation
                if isinstance(proposal, dict):
                    proposal["workflow"] = dict(workflow)
                return {
                    "runId": "powerbook-gemini-direct",
                    "skill": "powerbook-gemini-chapter",
                    "source": "gemini-3.1-pro-preview",
                    "workflow": workflow,
                    "events": [{"type": "powerbook.gemini.patch.ready", "workflow": workflow}],
                    "output": proposal,
                    "gemini": direct_summary,
                }
        ...
        if codex_result.get("error"):
            fallback_reason = fallback_reason or str(codex_result.get("error"))
        if codex_result.get("ok") and validation_is_valid(validation) and patch_has_changes(proposal):
            workflow["fallbackReason"] = fallback_reason or str(workflow.get("fallbackReason") or "")
            if isinstance(proposal, dict):
                proposal["workflow"] = dict(workflow)
            return {
                ...
                "source": "codex-app-server",
                "workflow": workflow,
                "events": [{"type": "powerbook.codex.patch.ready", "summary": codex_summary, "workflow": workflow}],
                "output": proposal,
                "codex": codex_summary,
            }

        workflow["fallbackReason"] = fallback_reason or (
            "codex_patch_failed_runtime_validation"
    ...
    def autonomous_workflow_start(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        runtime = self._require_runtime()
        assert self.project_root is not None
        context = runtime.refreshed_context()
        file_path = _optional_string(payload, "file") or _optional_string(payload, "chapter")
        ...
        if not file_path:
            raise ProjectLoadError("当前项目没有可修订章节。")
        safe_chapter_path(context.root, file_path)
        goal = _optional_string(payload, "goal") or _optional_string(payload, "instruction") or ""
        mode = _optional_string(payload, "mode") or "chapter-revision"
        timeout = _optional_number(payload, "timeoutSeconds") or DEFAULT_POWERBOOK_WORKFLOW_TIMEOUT_SECONDS
    ...
    def autonomous_workflow_status(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        self._require_runtime()
        assert self.project_root is not None
        return load_run_status(self.project_root, _required_string(payload, "runId"))

    def autonomous_workflow_artifacts(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        self._require_runtime()
        assert self.project_root is not None
        return load_run_artifacts(self.project_root, _required_string(payload, "runId"), include_text=_bool_payload(payload, "includeText"))

    def autonomous_workflow_preview(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        self._require_runtime()
        assert self.project_root is not None
        patch = payload.get("patch")
        if not isinstance(patch, Mapping):
            patch = load_run_patch(self.project_root, _required_string(payload, "runId"))
        return self.preview_patch({"patch": patch, "allowReviewed": _bool_payload(payload, "allowReviewed")})

    def autonomous_workflow_accept(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        self._require_runtime()
        assert self.project_root is not None
        patch = payload.get("patch")
        if not isinstance(patch, Mapping):
            patch = load_run_patch(self.project_root, _required_string(payload, "runId"))
        return self.apply_patch({"patch": patch, "allowReviewed": _bool_payload(payload, "allowReviewed")})
    ...
    def create_project(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        result = create_book_project(
            ...
            mode=_optional_string(payload, "mode") or _optional_string(payload, "projectMode") or "standard",
        )
        AuditLog(result["root"]).append({"type": "project.created", "title": result["plan"]["title"]})
        result.update(create_project_baseline_commit(result["root"], result["plan"]))
...
class BookWorkbenchHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        try:
            ...
            elif parsed.path == "/api/workflows/autonomous/start":
                self._send_json(self.app.autonomous_workflow_start(payload))
            elif parsed.path == "/api/workflows/autonomous/status":
                self._send_json(self.app.autonomous_workflow_status(payload))
            elif parsed.path == "/api/workflows/autonomous/artifacts":
                self._send_json(self.app.autonomous_workflow_artifacts(payload))
            elif parsed.path == "/api/workflows/autonomous/preview":
                self._send_json(self.app.autonomous_workflow_preview(payload))
            elif parsed.path == "/api/workflows/autonomous/accept":
                self._send_json(self.app.autonomous_workflow_accept(payload))
            elif parsed.path == "/api/workflows/powerbook/gemini-chapter":
                self._send_json(self.app.powerbook_gemini_chapter(payload))
...
def _powerbook_workflow_summary(root: Path) -> Dict[str, Any] | None:
    ...
    if import_path.exists():
        ...
        try:
            loaded = json.loads(guide_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            loaded = {}
        metadata = loaded if isinstance(loaded, dict) else {}
    ...
    if not status_counts and (root / "chapters").exists():
        status_counts["draft"] = len(list((root / "chapters").glob("*.md")))
    ...
    artifact_specs = [
        ...
        ("工作流说明", "WORKFLOW.md"),
        ("项目约定", "AGENTS.md"),
        ("自主工作流记忆", ".bookai/powerbook-memory.json"),
    ]
    ...
    return {
        "source": source,
        "sourceTreeHash": metadata.get("sourceTreeHash") if isinstance(metadata, dict) else "",
        "statusCounts": status_counts,
        "statusMapping": metadata.get("statusMapping", {}) if isinstance(metadata, dict) else {},
        "chapterTarget": _powerbook_target_chapter(root, metadata),
        "artifacts": artifacts,
    }


def _powerbook_target_chapter(root: Path, metadata: Mapping[str, Any]) -> str:
    target = metadata.get("chapterTarget") if isinstance(metadata, Mapping) else None
    if isinstance(target, str) and (root / target).exists():
        return target
    chapters = metadata.get("chapters") if isinstance(metadata, Mapping) else []
    if isinstance(chapters, list):
        targets = [item.get("target") for item in chapters if isinstance(item, dict) and isinstance(item.get("target"), str)]
        for candidate in targets:
            if re.search(r"ch0*1", candidate, re.I) and (root / candidate).exists():
                return candidate
        for candidate in targets:
            if not re.search(r"ch0*0", candidate, re.I) and (root / candidate).exists():
                return candidate
        for candidate in targets:
            if (root / candidate).exists():
                return candidate
    files = sorted((root / "chapters").glob("*.md")) if (root / "chapters").exists() else []
    for path in files:
        rel = path.relative_to(root).as_posix()
        if re.search(r"ch0*1", rel, re.I):
            return rel
    for path in files:
        rel = path.relative_to(root).as_posix()
        if not re.search(r"ch0*0", rel, re.I):
            return rel
