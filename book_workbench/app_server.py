...
<body data-app-title="书稿工作台本地版">
  ...
  <script>
    ...
    async function runPowerBookGeminiChapter(options = {}) { if (!state.project?.powerbookWorkflow) throw new Error("当前项目不是 PowerBook 导入项目。"); const first = firstChapterFile(); const file = state.currentFile || first; if (!file) throw new Error("当前项目没有可修订章节。"); const chapterBefore = await api("/api/chapters/" + encodeURIComponent(file)); state.currentFile = file; state.currentChapter = chapterBefore; toast(options.directGemini ? "正在通过受信任入口尝试调用 Gemini，并生成可审核修改建议。" : "正在按导入工作流生成整章修改建议。"); const result = await api("/api/workflows/powerbook/gemini-chapter", { method: "POST", body: JSON.stringify({ file, timeoutSeconds: 90, directGemini: !!options.directGemini, instruction: "按 PowerBook 原始 Codex/Gemini 流程修订当前章节；只输出 PatchProposal。" }) }); state.lastPatch = result.output; state.lastWorkflow = result.workflow || result.output?.workflow || null; state.currentChapter = chapterBefore; const workflow = state.lastWorkflow || {}; const invoked = workflow.geminiInvoked ? "已实际调用 Gemini" : "未实际调用 Gemini，已保留原因"; toast(`${invoked}，进入差异审核。`); await previewPatch(false); setView("diff"); return result; }
    ...
    function renderWorkflowEvidence() { const el = $("workflowEvidence"); if (!el) return; const workflow = state.lastWorkflow || state.lastPatch?.workflow; if (!workflow) { el.classList.add("hidden"); el.innerHTML = ""; return; } const invoked = workflow.geminiInvoked ? "是" : "否"; const source = workflow.source === "gemini-3.1-pro-preview" ? "Gemini 直连" : (workflow.source === "codex-app-server" ? "Codex 本地服务" : (workflow.source || "待确认")); const reason = workflow.fallbackReason ? `<div class="check-row"><span>!</span><span>未直连原因</span><strong>${escapeHtml(workflow.fallbackReason)}</strong></div>` : ""; const workflowName = workflow.name || "PowerBook Gemini 整章修订"; const log = workflow.runLog ? `<div class="check-row"><span>☰</span><span>运行日志</span><strong>${escapeHtml(workflow.runLog)}</strong></div>` : ""; el.classList.remove("hidden"); el.innerHTML = `<h3 class="mt">工作流证据</h3><div class="check-list"><div class="check-row"><span>▣</span><span>工作流</span><strong>${escapeHtml(workflowName)}</strong></div><div class="check-row"><span>✦</span><span>模型</span><strong>${escapeHtml(workflow.model || "gemini-3.1-pro-preview")}</strong></div><div class="check-row"><span>⌘</span><span>脚本</span><strong>${escapeHtml(workflow.scriptPath || "scripts/polish_chapters_gemini.py")}</strong></div><div class="check-row"><span>▣</span><span>实际调用 Gemini</span><strong class="${workflow.geminiInvoked ? "ok" : "bad"}">${invoked}</strong></div><div class="check-row"><span>→</span><span>建议来源</span><strong>${escapeHtml(source)}</strong></div>${reason}${log}</div>`; }
...
class RuntimeWebApp:
    ...
    def powerbook_gemini_chapter(self, payload: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if not powerbook_workflow_available(context.root):
            raise ProjectLoadError("当前项目不是 PowerBook 导入项目，不能运行 PowerBook 专用工作流。")
