...
<body data-app-title="BookWorkbench Local App">
  ...
  <script>
    ...
    async function openProject(relativePath, options = {}) { const payload = await api("/api/projects/open", { method: "POST", body: JSON.stringify({ relativePath }) }); state.project = payload.project; state.currentFile = null; state.currentChapter = null; state.lastPatch = null; state.lastPreview = null; await loadProjectDetails(true); setView("dashboard"); if (!options.quiet) toast(`已打开：${bookTitle()}`); return state.project; }
