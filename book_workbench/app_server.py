...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .metric { padding: 18px; min-height: 136px; min-width: 0; overflow: hidden; display: grid; grid-template-columns: 46px minmax(0, 1fr); align-items: center; gap: 14px; }
    ...
    .metric .label { color: var(--muted); font-size: 14px; line-height: 1.25; white-space: nowrap; word-break: keep-all; overflow-wrap: normal; }
    .metric .num { display: block; margin: 4px 0; font-size: 36px; line-height: 1; font-weight: 800; letter-spacing: -.04em; }
    .metric .note { display: block; font-size: 13px; line-height: 1.35; white-space: nowrap; word-break: keep-all; overflow-wrap: normal; overflow: hidden; text-overflow: ellipsis; }
    .metric-body { min-width: 0; width: 100%; overflow: hidden; display: grid; gap: 2px; }
    .table-scroll { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }
    .chapter-table { width: 100%; min-width: 720px; border-collapse: collapse; font-size: 14px; table-layout: auto; }
    ...
    .chapter-table th:nth-child(4), .chapter-table td:nth-child(4) { min-width: 88px; text-align: right; }
...
<body data-app-title="书稿工作台本地版">
  ...
  <script>
    ...
    function scopeLabel(value) { return ({ draft: "草稿", unreviewed: "未审阅", reviewed: "已审阅", locked: "已锁定" })[value] || ruleTypeLabel(value) || value || "全部"; }
    function annotationStatusLabel(status) { return ({ open: "未处理", resolved: "已处理", dismissed: "已忽略" })[status] || status || "未处理"; }
    function patchSourceLabel(source) { const value = String(source || ""); if (value.startsWith("AN-")) return `批注 ${value.slice(3)}`; if (value.startsWith("USER")) return "手动建议"; return value || "手动建议"; }
    function ruleDisplayLabel(rule) { const value = String(rule || ""); if (value.startsWith("R-")) return `规则 ${value.slice(2)}`; return value || "无"; }
    ...
    function fileDisplayLabel(file) { const files = Object.keys(state.project?.blocks || {}).sort(); const index = files.indexOf(file); if (index >= 0) return `第 ${index + 1} 章`; const match = String(file || "").match(/ch0*(\d+)/); return match ? `第 ${Number(match[1])} 章` : "当前章节"; }
    function annotationDisplayId(id) { const value = String(id || ""); return value.startsWith("AN-") ? `批注 ${value.slice(3)}` : (value || "批注"); }
